...
var activityID;//临时大文章ID
var activityReleaseID;//发布大文章ID
var articleType = {
		type:"",
		code:0,
		headImg:"",
		isArt:0,//0:大文章 1:活动 2:家庭征文 3:研学 4.课程5.公告6.感悟
		themeId:""
};
...
function checkArticle(ue){
	$("#articleErr").empty();
	var articleTitle = $("input#article_title").val();
	...
	if(!articleType.headImg){
		$("#articleErr").html("请添加封面！");
		showErrMsg("请添加封面！");
		return false;
	};
	if(!hasContent){
	...
	if(articleType.isArt == 1){
		...
		oDate = oDate.getTime();
		/*if(!signStartDate){
			$("#articleErr").html("请输入活动报名开始日期！");
			showErrMsg("请输入活动报名开始日期！");
			return false;
		};

		var oDate1 = new Date(signStartDate).getTime();		
		if(oDate1 < oDate || signStartDate.length > 10){
...
function saveArticle(ue,type){//type为保存类型 1:草稿箱 2:发布
	...
	}else if(articleType.isArt == 2){ //家庭征文
	...
	$.ajax({
		...
		success : function(x) {
			if(type == 1){
				...
				activityReleaseID = x;
				$("#articleErr").html("发布成功，可进入历史文章查看！");
...
function updateArticle(ue,type){//type为保存类型 1:草稿箱 2:发布
	...
	if(type == 2){
		_activityID = activityReleaseID; 
	};
	var current = new Date().Format("yyyy-MM-dd HH:mm:ss");
	var comment = "<div style='background:#FAFAFA;overflow:hidden;margin-top:8px;vertical-align:middle;'>" + 
	"<a class='button-grouped ys_article'><img class='cont_log' src='images/content.svg'/></a>" +
	"<div class='content_title'>" + articleTitle + "</div>" + "|<div>" + "</div>|" + 
	"<div class='U45_bd_article'><div id='#U45_text_0' class='text_content'>"+articleContent+"</div></div>" + "</div>";
	comment = comment + "<div class='ring_mes' onclick=\'U27_showBlog(\"" + roomId + "\",\"" + roomName + "\")\'>" + roomName + "的圈</div>";
	var data = {
			updateDate:current,
			title:articleTitle,
			context:articleContent,
...
function insertAudioBox(ue){
	...
	$('#audio_upload').on('change', function(){
		...
		var imgSize = fileList[0].size;
		if(imgSize > audioMaxSize){			
			$("div.insertAudioBox").find("div.errMsg").html("音频过大！音频大小限制"+(audioMaxSize/1024/1024)+"M");
			errorMsg = "音频过大！音频大小限制"+(audioMaxSize/1024/1024)+"M";
			return;
		};        
	});
...
function insertVideoBox(ue){
	...
	$('#audio_upload').on('change', function(){
		...
		if(imgSize > videoMaxSize){			
			$("div.insertAudioBox").find("div.errMsg").html("视频过大！视频大小限制"+(videoMaxSize/1024/1024)+"M");
			errorMsg = "视频过大！视频大小限制"+(videoMaxSize/1024/1024)+"M";
			return;
		};        
	});
...
$(function(){
	...
	var ganwuArticleId = (new getUrlParam()).ganwuArticleId;
	var roomId = (new getUrlParam()).roomId;
	var isLoad = sessionStorage.getItem("isLoad");
	//获取房间列表
	if(type == "activity"){
	...
	}else if(type == "familyactivity" || isLoad){
		$(".articleQuan .roomSel").html("请选择要发布的家庭：");
		getFamilyRoomList('newArticle');
		articleType.isArt = "2";
	...
	if(subject){
		articleType.code=101;
	};	
	getSubject("newArticle",subject);
	activityID = "";
	activityReleaseID = "";
	$("div.newArtCls").empty();
	ueId++;
	var editorDiv = "<div id='newArt" + ueId + "' style='width:100%;'></div>";

	$("div.newArtCls").append(editorDiv);
	var sectionHeight = $("section.new_article_section").height();
	var editHeight = sectionHeight - 300;
	//加载编辑器插件
	var ue = UE.getEditor("newArt" + ueId); 
	...
	//插入封面
	$(".new_article_section").on("click",".jfmbox",function(){
		var $imgBox = $(this);
