...
function getReleaseArticleList(){
	...
	$.ajax({
		url : SERVER_URL+"QueryActivityServlet",
		type : "POST",
		data : data,
		crossDomain : true,
		dataType : "json",
		success : function(x) {	
			...
			if(!dataLen){
				...
				for(var i=0;i<data.length;i++){
					...
					var edit = "<td><a class='editBtn' href='#/editArticle?type=2&editArticleId="+activityID+"&muactivityID=" + muactivityID +
					   			"&activity=1'>编辑</a></td>";
					
					var detail = "<td><a class='detailBtn' href='#/signArtitityList?title="+encodeURI(articleTitle)+"&editArticleId="+activityID+"&muactivityID=" + muactivityID +
								 "&sequence=" + data[i].sequence + "&mark1=" + encodeURI(data[i].mark1) + "&mark2=" + encodeURI(data[i].mark2) + 
								 "&mark3=" + encodeURI(data[i].mark3) + "&mark4=" + encodeURI(data[i].mark4) + "&mark5=" + encodeURI(data[i].mark5) + "'>报名详情</a></td>";					
					$tr.append(title);
...
$(function(){
	...
	//获取房间列表
	getAdminRoomList('historyArticleList');
	//查询文章并在页面显示列表
	getReleaseArticleList();	
	...
	//报名详情按钮
	$("#history_article_table").find("tbody").on("click","a.detailBtn",function(){
		loadMode("signArtitityList");
	});
});
